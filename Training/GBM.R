...
xtr <- df_x.train
xtr$eq_finitial <- as.character(xtr$eq_finitial)
xtr$eq_sinitial <- as.character(xtr$eq_sinitial)
xtr$eq_topic <- as.character(xtr$eq_topic)
xtr <- data.matrix(xtr)
...
# train model
xgbmod <- xgboost(xtr, as.vector(df_y.train), eta=1, nthread=8, max.depth=8, nrounds = 60, objective='binary:logistic')

# predict
xgbpre <- predict(xgbmod, xte)

# print measures
measures(xgbpre, df_y.test)

# importance of features
importance <- xgb.importance(feature_names = colnames(xtr), model = xgbmod)
# plot importance
