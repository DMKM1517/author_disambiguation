...
ind0 <- which(surnames[,"cat"]=='pctblack')
ind1 <- which(surnames[,"cat"]!='pctblack')
...
colnames(sur_race) <- c("name", "pctblack", "letters", "soundex")
...
tmp <- sur_race[c("soundex", "pctblack")]
...
saveRDS(svm_model, "../models/ethnic_black.rds")
