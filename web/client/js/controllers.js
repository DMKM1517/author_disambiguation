...
App.controller('HomeController', ['$scope', '$cookies', '$state', '$http', function($scope, $cookies, $state, $http) {
	var socket = io();
	const	prog_step = 100 / 14; // steps to add to progress bar
	$scope.logged_in = false; // logged in with mendeley
	$scope.mendeley_opened = true; // tab mendeley
	$scope.results_opened = false; // tab results
	$scope.processing = false; // processing the article
	$scope.progress = 0; // progress bar
	// test results
	...
	$scope.options_keywords = []; // no options for keywords
	$scope.config_keywords = {
		create: true,
		createOnBlur: true,
		hideSelected: true,
		selectOnTab: true,
	...
	$scope.options_subjects = []; // initially without options for subjects
	$scope.config_subjects = {
		create: false,
		hideSelected: true,
		selectOnTab: true,
	...
	$http.get('/getSubjects').then(function(resp) {
		if (resp.data) {
			$scope.options_subjects = resp.data.map(x => {
				return {
					value: x.subject,
					text: x.subject
				}
	...
	$scope.selectDocument = function(key) {
		...
		if (doc.keywords) {
			$scope.options_keywords = doc.keywords.map(x => {
				return {
					value: x,
					text: x
				};
	...
	// add author to the table of authors
	...
	// remove author from the table of authors
	$scope.removeAuthor = function(key) {
		$scope.article.authors.splice(key, 1);
	};

	// add reference to the table of references
	...
	// remove reference from the table of references
	...
	// on a message received from the server
